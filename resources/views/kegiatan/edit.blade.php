...
<form action="{{ route('kegiatan.update', $kegiatan) }}" method="POST" class="card p-4">
  @csrf
  @method('PUT')
  ...
  <div class="row">
    ...
    <div class="col-md-6 mb-3">
      ...
      <input type="date" name="tanggal_selesai" class="form-control" value="{{ old('tanggal_selesai', $kegiatan->tanggal_selesai->format('Y-m-d')) }}" required>
    </div>
  </div>
  ...
  <div class="mb-3">
    ...
    <select name="status" class="form-control" required>
      ...
      <option value="Tidak Aktif" {{ old('status', $kegiatan->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
    </select>
  </div>
